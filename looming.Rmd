...
```{r}
loomingcomb %>%
  filter(experiment=="even") %>% droplevels -> looming_even_example
looming_even_example %>% head(4)
```
Or similar commands or sub-setting to make new dataframes. Having multiple files can be confusing. 


I see that in the *even* experiment you have the three different experiments you mention, and one other:

```{r}
levels(looming_even_example$stimulus[looming_even_example$experiment=="even"])
```

Could you:

* remove spaces within names? - replace them with underscores
...
There are empty columns with one dataset because of how excel made the csv. You can go to the file in excel and delete the adjacent empty columns and make it again. Or you can open the file in R and replace all instances of ,,,,,,,, with nothing, as I have done. (Or remove this file, as I have suggested above.)
...
So, the black (intercept) control has an enormous impact
...
Whereas, this treatment (Even background) - I'm not quite sure what it is - elicits a response half the time
...
'convergence code 1 from optimx'
 ...
 I dunno. Perhaps has difficulty converging. 

Btw, John no longer recommends a violin plot for the summary stats. You can take that out if you like. 
...
```{r}
library(ggeffects)
ggpredict(loomingmodel.example4,'stimulus') %>% plot()
```
